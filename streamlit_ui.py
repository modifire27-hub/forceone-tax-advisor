    ...
    for r in doc_rounds:
        ...
        if r.get("external_ai_input"):
            ...
            if _src == "auto":
                ...
                st.markdown(f"**{r['round']}차 — 수동으로 붙여넣은 다른 AI의 답변**")
            else:
    ...
    if not st.session_state.get(proceed_key):
        ...
        if _show_auto:
            if not engine.auto_cross_check_enabled:
                ...
                st.caption(
                    "API 호출은 응답이 비정상 종료돼도 과금될 수 있습니다. 실행 중 "
...
@st.dialog("확정 지식베이스 조회", width="large")
def show_knowledge_base_dialog():
    """
    '검색 기록' 탭의 '지식베이스 조회' 버튼을 누르면 뜨는 모달 팝업.
    확정 지식베이스를 검색·목록으로 보고, 각 항목의 전체 내용을 확인한다.
    (검색 기록/종합 문서 상세보기 다이얼로그와 동일한 팝업 패턴)

    다이얼로그 안에서는 st.rerun()을 호출하지 않는다(호출하면 팝업이 닫힘).
    대신 필요한 경우 데이터를 즉시 다시 읽어와 아래에서 최신 상태로 렌더링한다.
    """
    if not (engine.sheet_logger and engine.sheet_logger.enabled):
    ...
    st.caption(
        "확정 저장된 문서를 검색합니다. 목록에는 요지(한 문장)와 키워드를 보여주고, "
        "각 항목을 펼치면 전체 확정내용을 볼 수 있습니다."
    )

    # 항상 최신 상태를 읽어와 표시(팝업을 다시 열면 갱신된 내용이 보이도록)
    kb_entries = engine.sheet_logger.get_recent_knowledge_entries(limit=1000)
    if not kb_entries:
        st.write("아직 확정된 지식베이스 항목이 없습니다.")
        return

    # 키워드·요지가 없는 항목이 있으면 일괄 생성 버튼 노출
    _missing = [
    ...
    if _missing:
        st.caption(
            ...
            "중간에 멈춰도 다시 눌러 이어서 처리합니다."
        )
        if st.button("기존 항목 키워드·요지 일괄 생성", key="kb_dlg_backfill_btn"):
            ...
            if not targets:
                ...
                # st.rerun() 대신 즉시 다시 읽어와 아래 목록을 최신 상태로 렌더링
                kb_entries = engine.sheet_logger.get_recent_knowledge_entries(limit=1000)

    kb_query = st.text_input(
        "검색어 (분류·질문·키워드·요지·내용에서 찾기)",
        key="kb_dlg_search_query",
    ...
    for idx, e in enumerate(_page_items, start=_start):
        ...
        with st.expander(_label, expanded=False):
            ...
            st.markdown("**확정내용:**")
            render_copyable_text(_content, key=f"kb_dlg_full_{idx}")
...
with tab_admin:
    if is_admin:
        ...
        # 지식베이스 다시 불러오기 (별도 섹션)
        # ------------------------------------------------------------------
        # AI는 답변할 때 지식베이스(로컬 _knowledge 파일 + 구글시트 '지식베이스' 탭)를
        # 참고하는데, 속도를 위해 한 번 읽은 내용을 메모리에 캐시해둔다. 그래서 다른
        # 사람이 방금 확정 저장한 내용은 내 화면의 캐시에 아직 없을 수 있다. 이 버튼은
        # 그 캐시를 강제로 비우고 최신 내용을 다시 읽어온다. 본인이 직접 확정 저장한
        # 경우에는 자동으로 갱신되므로 평소에는 누를 일이 거의 없다.
        st.divider()
        st.subheader("지식베이스 다시 불러오기")
        st.caption(
            "AI가 참고하는 지식베이스를 최신 상태로 다시 읽어옵니다. "
            "다른 사람이 방금 확정 저장한 내용이 내 화면에 아직 반영되지 않았을 때, "
            "또는 로컬 지식 파일을 수정해 올렸을 때 누르세요. "
            "본인이 확정 저장한 내용은 자동 반영되므로 평소에는 누를 필요가 없습니다."
        )
        if st.button("지식베이스 다시 불러오기", key="reload_kb_btn"):
