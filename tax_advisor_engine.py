...
- **표준 적요 가이드**: [기장 직원이 전표에 입력할 표준 적요 텍스트 예시. 실제
  ...
  "○○○○년 ○○월"이라는 8글자 고정 문구 하나를 통째로 사용하세요. 이 문구는
  "○○○○년"과 "○○월"을 더 채우거나 늘리지 않고 그대로 한 덩어리로만 쓰는
  것입니다 — "○○월"을 두 번 반복해서 적지 마세요. 차량번호 등 다른 가변
  정보도 "○○○○" 형태로 한 번만 표기하세요.
  올바른 예: "○○○○년 ○○월 차량유지비 지급 (차량번호 ○○○○)"
  잘못된 예 (절대 이렇게 쓰지 말 것): "○○○○년 ○○월 ○○월 차량유지비 지급"]
...
def _check_unreflected_items(original_content: str, corrected_content: str, mismatch_items: list) -> list:
    """
    D안 사후 안전장치 — _apply_corrections가 "수정했다"고 보고한 뒤, 실제로
    각 mismatch 항목에서 지적된 "원본의 틀린 핵심 문구"가 최종본에도 여전히
    그대로 남아있는지 코드로 직접 대조함.
...
"""
    ...
    def _apply_corrections(self, question: str, content: str, verification_text: str) -> tuple:
        ...
        변경 (C안 — 2026-06-26): 최종 수정본에서, 검증 결과를 반영해 실제로
        고친 부분(원본과 달라진 표현)을 마크다운 볼드(**...**)로 표시하도록
        지시함. 1차 호출(_run_verification_search)이 검증 결과에서 "답변과
        다름" 판정과 잘못된 표현을 볼드로 강조한 것과 짝을 이루는 표시로,
        회계사가 ① 검증 상세에서 무엇이 문제였는지, ② 최종본에서 실제로
        어디가 바뀌었는지를 각각 볼드만 보고 빠르게 대조 확인할 수 있게 함.

        변경 (D안 — 2026-06-26 추가, 검증 기능 고도화 3차 보강):
        - 실사용 테스트에서, 검증 결과 텍스트 전체를 모델에게 주고 "답변과
          다름 항목을 알아서 찾아 고치라"고 맡기면, 항목이 많아질수록 일부를
          누락하면서도 "전부 반영했다"고 잘못 보고하는 사례가 반복 확인됨.
        - 이를 막기 위해, "답변과 다름" 항목을 모델에게 다시 찾으라고 시키지
          않고 _extract_mismatch_items 헬퍼로 코드가 직접 미리 추출함.
          추출된 항목이 0개면 애초에 고칠 게 없으므로 모델 호출 자체를
          생략하고 원본을 그대로 반환함 (비용 절감 + "일치함"을 "답변과 다름"
          으로 착각해 불필요하게 건드릴 위험 자체를 차단).
          추출된 항목이 있으면, 그 목록만 별도로 작고 명확한 섹션으로
          fix_prompt 맨 앞에 제시해 모델의 작업을 "이 정해진 목록을 빠짐없이
          반영하는 것"으로 좁힘.
        - 추가 안전장치: 모델이 작업을 끝낸 뒤, 추출했던 각 항목의 핵심
          문구(원본에서 틀렸다고 지적된 부분)가 최종본에 여전히 그대로
          남아있는지 코드로 다시 확인함(_check_unreflected_items). 하나라도
          그대로 남아있으면, 회계사가 놓치지 않도록 correction_summary 맨
          앞에 경고 문구를 강제로 덧붙임 — "수정했다"는 모델의 자체 보고를
          무조건 신뢰하지 않고, 코드가 한 번 더 사실을 대조하는 구조.
        ...
        # 추출된 "답변과 다름" 항목이 하나도 없으면, 고칠 내용이 없다는 뜻이므로
        # 모델을 호출할 필요 자체가 없음. 원본을 그대로 반환함.
        # (단, verification_text 자체가 비정상적으로 짧거나 형식이 깨진 경우는
        # "0개"가 "정말로 문제가 없어서 0개"인지 "파싱이 실패해서 0개"인지
        # 구분이 안 되므로, 검증 텍스트가 일정 길이 이상일 때만 이 단축 경로를
        # 사용함 — 너무 짧으면 안전하게 기존 방식(모델에게 전체를 맡기는 방식)으로 진행)
        ...
        if mismatch_items:
            mismatch_block = (
                "[반드시 반영해야 할 항목 목록 — 아래 "
                f"{len(mismatch_items)}개 항목을 빠짐없이 전부 반영하세요. "
                "하나라도 빠뜨리면 안 됩니다]\n\n"
                + "\n\n".join(f"{i}. {item}" for i, item in enumerate(mismatch_items, start=1))
            )
        else:
            # verification_text가 너무 짧아 위 단축 경로를 타지 않은 경우의 폴백.
            # 이 경우는 실제로 항목이 있을 수도 있으니 모델에게 전체를 맡김.
            mismatch_block = "(주의: 검증 결과에서 '답변과 다름' 항목을 자동으로 추출하지 못했습니다. 아래 검증 결과 전체를 직접 검토해 반영하세요.)"
...
[검증 결과 전체 — 참고용. 위의 "반드시 반영해야 할 항목 목록"이 이 안에서
추출된 것이며, 위 목록이 작업 대상의 전부입니다. 아래 전체 텍스트에서 추가로
"답변과 다름"을 더 찾으려 하지 마세요 — 그러면 오히려 "일치함"이나 "확인 못함"
항목을 잘못 건드릴 위험이 있습니다]
{verification_text}

[작업 지침]
1. 위 "반드시 반영해야 할 항목 목록"에 있는 항목을 하나씩, 번호 순서대로
   확인하세요. 목록의 항목 개수와, 실제로 고친 항목 개수가 정확히 일치해야
   합니다.
2. 각 항목에 적힌 정확한 내용(검색으로 확인된 정답)으로 원본의 해당 부분을
   고치세요. 본문뿐 아니라 "4. 관련 근거 및 유의사항"의 법령 근거 목록처럼
   같은 내용이 문서 여러 곳에 반복 등장하는 경우, 그 모든 위치를 빠짐없이
   고치세요 — 본문만 고치고 법령 근거 목록은 그대로 두는 식으로 절반만
   고치면 안 됩니다.
3. 위 목록에 없는 부분(즉 검증에서 "일치함"이거나 "확인 못함"으로 남은 부분)은
   원본 표현을 그대로 유지하세요. 다만 "확인 못함" 부분이 과도하게 단정적인
   어조라면 "최신 법령 확인 필요" 정도로 조심스럽게 표현하세요.
4. 마크다운 구조(### 1. 질의 요지, ### 2. 세무 결론 및 판단 등)는 그대로 유지하세요.
5. [필수 — 작업 완료 후 자체 점검] 새로 작성한 전체 텍스트를 위 "반드시
   반영해야 할 항목 목록"과 번호 하나씩 다시 대조하세요. 목록에 있던 원본의
   틀린 표현(조항 번호, 단정적 문장 등)이 새 텍스트 어디에도 단 한 글자도
   남아있지 않아야 합니다. 하나라도 그대로 남아있다면 그 항목을 다시 고쳐서
   반영하세요. 이 자체 점검을 건너뛰지 마세요.
6. [중요 — 볼드 강조 규칙] 위 목록의 항목을 반영해 실제로 고친 부분은,
   고쳐 쓴 새 표현을 마크다운 볼드(**고친 표현**)로 감싸서 표시하세요.
   회계사가 전체 문서를 다 읽지 않고도 굵은 글씨만 보고 "여기가 바뀐
   부분이구나"를 바로 알아볼 수 있어야 합니다. 볼드는 고친 핵심 단어/구절
   에만 짧게 적용하세요(문장 전체를 볼드로 감싸지 마세요). 원본을 그대로
   둔 부분에는 볼드를 쓰지 마세요.

[출력 형식 — 정확히 이 두 구분자를 사용하세요]

[수정된전체본]
(마크다운 구조를 유지한 채로, "반드시 반영해야 할 항목 목록"의 모든 항목이
문서 내 모든 위치에서 반영된 전체 답변 텍스트. 실제로 고친 표현은 위 6번
규칙대로 **볼드**로 표시)
...
"""
        ...
        # D안 — 사후 안전장치: 추출했던 "답변과 다름" 항목들이 실제로 반영됐는지
        # 코드로 한 번 더 확인. 모델의 "수정했다"는 자체 보고를 무조건 신뢰하지
        # 않고, 원본에서 지적된 틀린 핵심 문구가 최종본에 그대로 남아있는지
        # 직접 대조함.
        if mismatch_items and corrected_content:
            unresolved = _check_unreflected_items(content, corrected_content, mismatch_items)
            if unresolved:
                warning = (
                    "[자동 점검 경고] 검증에서 '답변과 다름'으로 지적된 내용 중 "
                    f"{len(unresolved)}건이 수정본에서도 그대로 남아있는 것으로 "
                    "보입니다 (자동 점검은 참고용이며 오탐이 있을 수 있습니다). "
                    "반드시 아래 항목을 직접 확인해 수동으로 고쳐주세요:\n"
                    + "\n".join(f"  - {snippet}" for snippet in unresolved)
                    + "\n\n"
                )
