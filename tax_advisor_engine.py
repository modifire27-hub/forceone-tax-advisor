...
"""
    ...
    def call_claude_verifier(self, prompt: str, enable_web_search: bool = True) -> tuple:
        """
        ...
        enable_web_search : bool
            웹검색 도구 사용 여부. True면 조문으로 부족할 때 Claude가 스스로
            국세청·법제처 등을 검색하도록 허용한다. 도구가 거부되면 자동으로
            도구 없이 한 번 더 시도한다.
    ...
    def run_auto_cross_check(
        ...
        dict
            {
              ...
              "stop_reason": str,    # "claude_complete" | "converged" | "max_rounds" | "error"
        ...
        for _ in range(max(1, max_rounds)):
            ...
            critique, is_complete, err = self.call_claude_verifier(prompt)
            result["rounds_run"] += 1
            if critique:
