        ...
        dict
            {
                ...
                "corrected_content": str,  # AI가 의심 포인트를 직접 고친 최종본
            ...
            corrected_content는 원본(content)을 그대로 반환하며 correction_summary에
            실패 사유를 안내함 (검증/수정 실패가 저장 자체를 막지는 않음 — 최종 판단은
            항상 회계사의 PIN 승인).

        설계 의도 (2026-06-25 추가 — 검증 결과 자동 반영):
        - 기존에는 검증 결과를 텍스트로만 보여주고, 회계사가 그 내용을 읽고 원문에서
          직접 해당 부분을 찾아 손으로 고쳐야 했음. 실제 사용 중 같은 수정을 여러
          번 반복해야 하는 불편이 확인됨.
        - 개선: AI가 검증과 동시에 "의심되는 부분을 직접 고친 최종본"까지 만들어서
          반환함. 회계사는 수정 요약(어디를, 왜 고쳤는지)만 빠르게 확인하고 동의하면
          되고, 원하면 여전히 텍스트칸에서 추가로 손을 볼 수 있음(자동 수정이
          100% 정확하다고 보장할 수 없으므로, 최종 검토 단계는 항상 유지함).
        """
...
Google 검색으로 이 답변의 핵심 주장들이 실제 법령/실무와 맞는지 다시 한번 점검하고,
문제가 있는 부분은 직접 고친 최종본을 만들어주세요.
...
1단계 — 검증 대상 목록화:
먼저 답변 안에서 검증이 필요한 모든 항목을 빠짐없이 나열하세요. 다음 종류를 모두
...
3단계 — 불일치 확인 및 수정:
검색 결과와 답변 내용이 다른 항목을 모두 찾아, 검색으로 확인한 정확한 내용으로
직접 고쳐서 답변 전체의 마크다운 구조(### 1. 질의 요지 등)는 그대로 유지한
"수정된 최종본"을 만드세요. 특히 (d)처럼 답변이 여러 하위 케이스를 하나로
뭉뚱그린 경우, 실제로는 케이스별 결론이 다르다면 그 구분을 명시적으로 추가하세요.
문제가 전혀 없으면 원본과 동일한 내용을 그대로 최종본으로 제시하세요.

4단계 — 검증되지 않은 부분 처리:
추측하지 말고, 실제로 검색해서 확인한 내용만 근거로 수정하세요. 확실하지 않으면
고치지 말고 원문을 그대로 두되, [검증결과]에 "이 부분은 검색으로 명확히 확인하지
못했습니다"라고 솔직하게 밝히세요. "특이사항 없음"은 실제로 검색해서 일치를
확인했을 때만 쓰세요 — 검색을 안 했거나 결과가 불충분해서 판단을 못 한 경우에는
절대 "특이사항 없음"이라고 쓰면 안 됩니다.

[출력 형식 — 아래 순서와 4개 구분자를 정확히 사용하세요. 순서가 중요합니다]

중요: 반드시 [수정된최종본]을 가장 먼저 작성하세요. 1~4단계에서 찾은 모든 불일치
사항은 예외 없이 이 [수정된최종본]에 직접 반영되어야 합니다. 나중에 쓰는
[검증결과]와 [수정요약]은 이미 완성된 [수정된최종본]을 보고 "내가 무엇을
왜 고쳤는지"를 설명하는 것일 뿐이며, [검증결과]에서 문제를 발견했다고 적어놓고
[수정된최종본]에는 그 문제가 그대로 남아있는 것은 절대 허용되지 않는 오류입니다.
[수정된최종본]을 다 쓴 뒤에는, 1단계에서 나열했던 항목들을 하나씩 다시 확인하며
정말로 전부 반영되었는지 스스로 재검토하세요.

[수정된최종본]
(마크다운 구조를 유지한, 1~4단계에서 찾은 모든 불일치 사항이 빠짐없이 직접
고쳐진 전체 답변 텍스트. 수정 사항이 전혀 없었다면 원본과 동일한 내용을 그대로
적으세요. 이 섹션을 다 쓴 후, 아래 [검증결과]에서 "답변 내용과 다름"이라고 쓸
모든 항목이 이미 여기에 반영되어 있는지 다시 한번 확인하세요.)

[검증결과]
(위 [수정된최종본]을 만들기 위해 검토한 항목을 모두 나열하고, 항목별로 검색
확인 결과를 짧게 표시하세요. 예:
- 법인세법 시행령 제43조 제2항 → 검색으로 확인, 일치함
- 건강보험 연체료 손금불산입 여부 → 검색으로 확인, 원본과 다름 → 최종본에 반영함
  (건강보험은 연체금·가산금 모두 손금불산입이나 원본은 보험 종류 구분 없이 서술함)
- 국민연금 연체료 → 검색으로 확인, 원본과 다름 → 최종본에 반영함 (연체금은
  손금산입·가산금만 손금불산입이나 원본은 이 구분이 없었음)
이렇게 검증한 항목 전부를 빠짐없이 나열하고, "답변 내용과 다름"이라고 적은
항목은 모두 "→ 최종본에 반영함"이 함께 적혀 있어야 합니다. 반영하지 않았다면
그 사실과 이유를 명시하세요.)

[수정요약]
(위 [수정된최종본]에서 원본과 비교해 실제로 달라진 부분을 1~3줄로 짧게 요약.
예: "약국을 면세사업자로 단정한 부분을 과세·면세 겸영사업자로 수정하고, 회식비는
안분 계산 대상이라는 점을 명시함". 고친 부분이 없으면 "수정 사항 없음"이라고 작성)

[추천파일]
(다음 중 하나를 정확히 그대로 적으세요: {options_text})
...
"""
        ...
        # 구분자 기반으로 각 섹션을 파싱. 모델이 형식을 어긴 경우를 대비해
        # 항상 안전한 기본값으로 폴백함 (절대 빈 화면이나 에러로 이어지지 않게).
        ...
        # 구분자 순서: [수정된최종본] → [검증결과] → [수정요약] → [추천파일]
        # (출력 순서를 이렇게 바꾼 이유는 위 프롬프트의 "중요:" 설명 참고 — 모델이
        # 먼저 최종본을 확정하고, 그 다음에 그걸 설명하게 해서 "검증에서는 문제를
        # 찾았는데 최종본에는 반영 안 됨" 같은 자기 모순을 줄이기 위함)
        markers = ["[수정된최종본]", "[검증결과]", "[수정요약]", "[추천파일]"]
        corrected_content = _extract_section(full_text, "[수정된최종본]", markers[1:])
        verification_text = _extract_section(full_text, "[검증결과]", markers[2:])
        correction_summary = _extract_section(full_text, "[수정요약]", markers[3:])
        recommend_block = _extract_section(full_text, "[추천파일]", [])

        if not full_text:
            verification_text = f"[검증 실패] 웹검색 기반 재검증 호출에 실패했습니다: {call_error}"
            correction_summary = "검증 호출에 실패하여 자동 수정을 수행하지 못했습니다. 원본 내용을 그대로 사용합니다."
            corrected_content = content
        elif not corrected_content:
            # 모델이 형식을 어겨 [수정된최종본] 섹션을 못 찾은 경우, 안전하게 원본 유지
            corrected_content = content
            if not correction_summary:
                correction_summary = "수정본을 명확히 파싱하지 못했습니다. 원본 내용을 그대로 사용하니 직접 검토해주세요."
        if not verification_text:
            verification_text = "검증 결과를 명확히 파싱하지 못했습니다. 직접 내용을 검토해주세요."
        if not correction_summary:
            correction_summary = "수정 사항 없음"

        # 추천 파일/이유 파싱 (recommend_block의 첫 줄=파일명, 둘째 줄부터=이유)
        recommended_file = self.KNOWLEDGE_FILE_OPTIONS[0]
        recommended_reason = "추천 사유를 확인하지 못했습니다 — 기본값으로 지정되었습니다."
        if recommend_block:
            lines = [l.strip() for l in recommend_block.split("\n") if l.strip()]
            if lines:
                candidate = lines[0]
