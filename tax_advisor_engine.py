...
[출력 형식 — 아래 4개 구분자를 정확히 사용하세요]

[검증결과]
(1단계에서 나열한 검증 대상 항목을 모두 나열하고, 항목별로 검색 확인 결과를
짧게 표시하세요. 예:
- 법인세법 시행령 제43조 제2항 → 검색으로 확인, 일치함
- 건강보험 연체료 손금불산입 여부 → 검색으로 확인, 답변 내용과 다름 (건강보험은
  연체금·가산금 모두 손금불산입이나 답변은 보험 종류 구분 없이 서술함)
- 국민연금 연체료 → 검색으로 확인, 연체금은 손금산입·가산금만 손금불산입이나
  답변은 이 구분이 없음
이렇게 검증한 항목 전부를 빠짐없이 나열하세요. 일부 항목만 적고 나머지를
생략하면 안 됩니다.)

[수정요약]
(이번에 실제로 고친 부분을 1~3줄로 짧게 요약. 예: "법인세법 시행령 제43조 항
번호를 제1항에서 제2항으로 수정함 (정관 한도초과 상여금 손금불산입 조항은
제2항이 맞음)". 고친 부분이 없으면 "수정 사항 없음"이라고 작성)

[수정된최종본]
(마크다운 구조를 유지한, 의심 포인트가 고쳐진 전체 답변 텍스트. 수정 사항이
없었다면 원본과 동일한 내용을 그대로 적으세요)
...
"""
        ...
        markers = ["[검증결과]", "[수정요약]", "[수정된최종본]", "[추천파일]"]
        verification_text = _extract_section(full_text, "[검증결과]", markers[1:])
        correction_summary = _extract_section(full_text, "[수정요약]", markers[2:])
        corrected_content = _extract_section(full_text, "[수정된최종본]", markers[3:])
