...
"""
    ...
    def build_cross_check_prompt(
        self, question: str, content: str, verification_text: str
    ) -> str:
        """
        ...
        설계 의도:
        - 단순히 "이게 맞아?"라고 묻는 것보다, 원본 답변 전체와 1차 웹검색
          검증 결과를 모두 포함시켜서, 다른 AI가 충분한 맥락을 가지고
          교차검증할 수 있게 함. 이렇게 하면 다른 AI도 처음부터 다시 찾는
          것이 아니라, 우리 시스템이 이미 찾은 의심 지점을 중심으로 더
          깊게 검증해줄 수 있음.
        - 이 메서드는 AI 호출이 전혀 없는 순수 텍스트 조합 함수임 (비용 없음,
          즉시 반환).
        ...
        verification_text : str
            _run_verification_search 등으로 얻은 1차(또는 가장 최근) 검증 결과 텍스트
...
[1차 웹검색 검증 결과 — 이미 의심되는 부분이 표시되어 있습니다]
{verification_text}

[요청 사항]
1. 위 "1차 웹검색 검증 결과"에서 "답변과 다름"으로 표시된 항목들이 실제로
   맞는지 직접 검색하거나 알고 있는 지식으로 다시 확인해주세요.
2. 1차 검증이 놓쳤을 수 있는 다른 오류(법령 조항 번호, 수치, 단정적 결론 등)도
   추가로 점검해주세요.
    ...
    def _apply_corrections(self, question: str, content: str, verification_text: str) -> tuple:
        ...
        for item in mismatch_items:
            ...
            if match_result is None:
                failed_items.append(
                    f"(원문에서 정확히 일치하는 위치를 찾지 못함) \"{original_snippet[:60]}...\""
                )
                continue
