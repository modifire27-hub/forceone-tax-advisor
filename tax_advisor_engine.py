...
"""
    ...
    def _apply_corrections(self, question: str, content: str, verification_text: str) -> tuple:
        ...
        for item in mismatch_items:
            ...
            if original_snippet not in corrected_content:
                # 모델이 인용한 발췌가 실제 원본에 한 글자도 안 틀리고 존재하지
                # 않으면 치환할 수 없음. 이 경우를 숨기지 않고 그대로 보고함 —
                # "반영했다고 말했지만 실제로는 안 됨"이 아니라 "반영을
                # 시도했으나 위치를 못 찾아 실패함"을 명확히 구분해서 전달.
                failed_items.append(
            ...
            corrected_content = corrected_content.replace(
                original_snippet, f"**{replacement}**"
            )
            applied_summaries.append(short_reason or f"\"{original_snippet[:30]}...\" 수정")
