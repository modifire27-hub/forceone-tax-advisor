...
2. 검색으로 확인한 결과와 답변 내용이 다른 부분을 찾으세요.
3. 문제가 있는 부분은 검색으로 확인한 정확한 내용으로 직접 고쳐서, 답변 전체의
   마크다운 구조(### 1. 질의 요지 등)는 그대로 유지한 "수정된 최종본"을 만드세요.
   문제가 없으면 원본과 동일한 내용을 그대로 최종본으로 제시하세요.
4. 추측하지 말고, 실제로 검색해서 확인한 내용만 근거로 수정하세요. 확실하지 않으면
   고치지 말고 원문을 그대로 두세요.

[출력 형식 — 아래 4개 구분자를 정확히 사용하세요]

[검증결과]
(의심되는 부분이 있었는지, 있었다면 무엇이 어떻게 틀렸는지를 검색 근거와 함께
간단히 설명. 문제가 없었다면 "검색으로 확인한 결과 특이사항 없음"이라고 작성)
