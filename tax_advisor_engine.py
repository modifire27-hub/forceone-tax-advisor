...
7. 검증 결과에 "답변과 다름" 항목이 전혀 없다면, 원본과 동일한 내용을 그대로
   다시 작성하세요.
8. [중요 — 볼드 강조 규칙] "답변과 다름" 항목을 반영해 실제로 고친 부분은,
   고쳐 쓴 새 표현을 마크다운 볼드(**고친 표현**)로 감싸서 표시하세요.
   회계사가 전체 문서를 다 읽지 않고도 굵은 글씨만 보고 "여기가 바뀐
   부분이구나"를 바로 알아볼 수 있어야 합니다. 볼드는 고친 핵심 단어/구절
   에만 짧게 적용하세요(문장 전체를 볼드로 감싸지 마세요). "일치함"이나
   "확인 못함"이라 원본을 그대로 둔 부분에는 볼드를 쓰지 마세요 — 볼드는
   오직 이번에 실제로 내용이 바뀐 자리에만 사용합니다.

[출력 형식 — 정확히 이 두 구분자를 사용하세요]

[수정된전체본]
(마크다운 구조를 유지한 채로, 검증 결과의 모든 "답변과 다름" 항목이 반영된
전체 답변 텍스트. 실제로 고친 표현은 위 8번 규칙대로 **볼드**로 표시)
