...
def _coerce_json_string(s: str) -> str:
    """
    JSON 문자열 리터럴 "내부"에 들어 있는, 이스케이프되지 않은 제어문자
    (줄바꿈/캐리지리턴/탭)를 규격에 맞는 이스케이프(\\n, \\r, \\t)로 바꿔줌.

    배경 (2026-07-10 추가): Gemini는 긴 문서를 JSON으로 낼 때 "줄바꿈은 \\n으로
    표시하라"는 프롬프트 지시를 자주 무시하고, 문자열 값 안에 실제 줄바꿈을
    그대로 넣음. 그러면 표준 json.loads가 "Invalid control character"로 실패함.
    특히 apply_external_feedback처럼 문서 전체(수천 자)를 담는 경우에 자주 발생.
    이 함수는 문자열 리터럴 안에 있을 때만 제어문자를 이스케이프하고, 구조적
    공백(키/값 사이 줄바꿈 등)은 건드리지 않아, 원래 유효했던 JSON은 그대로 두고
    깨진 것만 살려냄. (문자열 밖의 줄바꿈은 json이 원래 허용하므로 손대지 않음)
    """
    out = []
    in_string = False
    escaped = False
...
def _parse_model_json(raw_text: str):
    """
    Gemini가 낸 JSON 응답을 "관대하게" 파싱해서 dict로 돌려줌. 실패하면 None.

    이 프로젝트의 여러 보조 호출(_assess_followup_research_need,
    apply_external_feedback, _resolve_single_mismatch, _resolve_mismatch_by_keyword)이
    똑같은 방식으로 모델 JSON을 파싱하는데, 예전에는 각자 코드펜스만 벗겨내고
    json.loads를 바로 호출해서, 모델이 조금만 형식을 어겨도(문자열 안 줄바꿈,
    앞뒤에 설명 문장을 덧붙임 등) 파싱이 실패했음. 그 실패가 곧 "라운드가
    조용히 무반영됨"으로 이어졌기 때문에, 파싱을 한 곳에 모아 견고하게 만듦.

    처리 순서:
      1) 코드펜스(```json ... ``` 또는 ``` ... ```) 제거
      2) 앞뒤에 설명 문장이 붙은 경우 대비 — 첫 '{' 부터 마지막 '}' 까지만 추출
      3) 1차: 그대로 json.loads
      4) 2차: 문자열 내부의 이스케이프 안 된 줄바꿈/탭을 고쳐서 재시도
    """
    import json
    if not raw_text:
        return None
    ...
    if cleaned.startswith("```"):
        parts = cleaned.split("```")
        # 보통 parts[1]이 코드 내용. 형식이 어긋나면 원본 유지.
    ...
    # 3) 1차 시도 (정상 JSON이면 여기서 끝)
    try:
        return json.loads(cleaned)
    except (json.JSONDecodeError, ValueError):
        pass

    # 4) 2차 시도 — 문자열 내부의 이스케이프 안 된 제어문자 구제
    try:
        return json.loads(_coerce_json_string(cleaned))
    except (json.JSONDecodeError, ValueError):
        return None
...
class TaxAdvisorEngine:
    ...
    def __init__(self, knowledge_dir: Path = None, env_path: Path = None):
        ...
        # 6. Claude(Anthropic) 자동 교차검증 클라이언트 (선택 기능 — v1.11 신규)
        #
        # 설계 의도:
        # - 지식베이스 확정 절차에서, 회계사가 외부 AI(ChatGPT 등)에 답변을 복사해
        #   붙여넣고 그 답을 다시 시스템에 넣던 "수동 교차검증"을, 서로 다른 벤더인
        #   Claude(Sonnet)를 API로 직접 호출해 자동으로 반복하도록 하기 위한 것.
        # - 저자 모델(Gemini)과 다른 벤더를 검증자로 두는 것이 핵심 — 같은 계열
        #   모델은 사전 믿음(편향)을 공유해 서로의 오류를 못 잡는 경우가 있음.
        # - ANTHROPIC_API_KEY가 없으면 자동 교차검증만 비활성화되고, 기존 수동
        #   교차검증 경로와 나머지 기능은 그대로 정상 동작함(하위 호환).
        # - 이 저장소는 공개(public)이므로 키는 절대 코드에 넣지 않고 Streamlit
        #   Secrets(또는 로컬 .env)에서만 읽음.
        self.anthropic_api_key = os.getenv("ANTHROPIC_API_KEY", "").strip()
        # 검증자 모델 문자열도 Secrets(CLAUDE_MODEL)로 빼서 코드 수정 없이 교체 가능.
        # 기본값은 현재 세대 Sonnet. 호출이 "model not found"로 실패하면 Anthropic
        # 문서(docs.claude.com)에서 정확한 모델 문자열을 확인해 CLAUDE_MODEL Secret으로
        # 지정하면 됨.
        self.claude_model = os.getenv("CLAUDE_MODEL", "claude-sonnet-5").strip()
        self._anthropic_client = None  # 첫 사용 시 지연 초기화

    @property
    def auto_cross_check_enabled(self) -> bool:
        """Claude 자동 교차검증 사용 가능 여부(키가 설정되어 있는지)."""
...
"""
        ...
        if parsed is None:
            print(f"[경고] 꼬리질문 리서치 판단 결과 JSON 파싱 실패 / 원본: {raw_text[:200]}")
            return fallback
...
"""
    ...
    # ------------------------------------------------------------------
    # v1.7 신규 (2026-06-28) — "본문이 매 라운드 실제로 보이고 바뀌는" 구조
    # ------------------------------------------------------------------
    # 배경: v1.6의 교차검증 스레드는 라운드를 여러 번 돌려도 "검증 텍스트"만
    # 누적되고 본문(실제 답변 문서)은 맨 마지막 "확정 단계로 진행"을 누를
    # 때 딱 한 번만 바뀌는 구조였음. 그 결과: (1) 회계사가 중간 라운드에서
    # 실제 문서가 어떻게 바뀌어가는지 확인할 수 없었고, (2) 외부 AI가
    # 2~4차에서 지적한 보완사항(법인 설명을 더 명확히, 법률 용어 사용 등)이
    # 정작 본문에는 전혀 반영되지 않은 채 누락됨 — 1차에서 찾은 오류만
    # 기계적으로 치환된 결과물이 나와, "라운드를 여러 번 거쳤는데 결과물은
    # 부실하다"는 회계사 피드백으로 이어짐.
    #
    # 변경: 1라운드는 기존처럼 웹검색 동반 검증+자동수정(verify_before_confirm)
    # 으로 "확정 문서 v1"을 만듦. 2라운드부터는 웹검색을 다시 하지 않고
    # (회계사 피드백 반영 — 웹검색은 느려서 1회만), 외부 AI의 자유 형식
    # 의견을 직접 받아 "문서 전체"를 다시 쓰는 방식(apply_external_feedback)
    # 으로 v2, v3...을 만듦. 매 라운드 "그 시점의 문서 전체"와 "이번에 뭘
    # 고쳤는지"가 화면에 그대로 보이고, 다음 라운드 외부 AI 질문에도 포함됨.
    def summarize_document_changes(
        self, original_document: str, revised_document: str
    ) -> str:
        """
        두 문서(주로 "원본 답변"과 "1차 확정 문서")를 직접 비교해서, 정확히
        무엇이 어떻게 바뀌었는지 자세한 변경 요약을 만듦.

        설계 의도 (2026-06-28 신규 — 회계사 피드백 반영):
        - 1라운드(verify_before_confirm)가 만드는 correction_summary는
          _apply_corrections 내부에서 항목별로 만들어진 짧은 사유들을 이어
          붙인 것이라, "원본과 1차 문서 전체를 놓고 비교했을 때 무엇이
          바뀌었는지"를 한눈에 보여주지 못함(예: "조항 번호를 제48조 제3항으로
          수정" 같은 항목 단위 사유만 나열됨). 회계사가 "결과적으로 어디가
          바뀐 건지 비교가 안 된다"고 지적함.
        - 2라운드부터 쓰는 apply_external_feedback은 이미 "무엇을 어떻게
          고쳤는지" 자체적으로 change_summary를 생성하므로 문제가 적었음.
          1라운드에만 빠져 있던 이 비교 단계를 별도로 채움.
        - 이 메서드는 verify_before_confirm 호출 직후, "원본 답변"과
          "verify_before_confirm이 만든 corrected_content"를 입력으로 받아
          호출됨. 두 문서를 나란히 놓고 실제로 달라진 부분만 비교하므로,
          apply_external_feedback과 마찬가지로 "없던 내용을 새로 만들어
          끼워 넣지 않도록" 하는 안전장치가 동일하게 필요함 — 이 메서드는
          본문을 수정하는 게 아니라 이미 일어난 변경을 설명만 하는 것이므로
          위험은 적지만, 모델이 "이것도 바뀌었어야 한다"는 식으로 실제로는
          안 일어난 변경을 있는 것처럼 서술할 위험은 여전히 있어 명시적으로
          금지함.

        Parameters
        ----------
        original_document : str
            비교 기준이 되는 이전 버전 문서 (보통 맨 처음 원본 답변)
        revised_document : str
            비교 대상이 되는 새 버전 문서 (보통 1차 확정 문서)

        Returns
        -------
        str
            불릿("- ") 형식의 한국어 변경 요약. 실제로 달라진 부분이 없으면
            "- 변경 사항 없음"을 반환. 호출 실패 시 실패 사유를 담은 문자열을
            반환함(문서 자체에는 영향 없음 — 화면 표시용 텍스트일 뿐).
        """
        prompt = f"""다음 두 문서를 직접 비교해서, 두 번째 문서가 첫 번째 문서와 비교해
정확히 무엇이 어떻게 달라졌는지 정리해주세요.
...
[작업]
1. 두 문서를 항목/문장 단위로 비교해서, 실제로 내용이 달라진 부분만
   찾아주세요. 단어 하나(예: 조항 번호, 금액, 표현)만 바뀐 것도 빠짐없이
   포함하세요.
2. 매우 중요 — 실제로 일어난 변경만 서술하세요. 두 문서를 비교해서 실제로
   다른 부분만 찾아 그대로 보고하세요. "이것도 바뀌었어야 한다"거나 "이
   부분도 검토가 필요하다" 같은, 비교와 무관한 의견이나 제안을 추가하지
   마세요 — 이 작업은 평가가 아니라 객관적인 대조입니다.
3. 변경이 없는 항목은 언급하지 마세요. 비교 결과만 간결하게 불릿으로
   나열하세요.
4. 각 불릿은 "(어느 부분이) (어떻게) (무엇으로) 변경됨" 형식으로, 1줄
   내외로 간결하게 쓰세요. 예: "- 법령 근거 중 '제5조 제4항'이 '제49조
   제1항'으로 변경됨"
5. 실제로 아무 차이가 없다면, "- 변경 사항 없음"이라고만 답하세요.

[출력 형식 — 불릿 목록만 출력하세요. 다른 설명이나 인사말 없이 바로
불릿으로 시작하세요]
"""
        try:
            response = self.client.models.generate_content(
            ...
            result = response.text.strip() if response.candidates else ""
        except Exception as e:
            return f"- [비교 실패] 변경 사항을 비교하는 중 오류가 발생했습니다: {e}"

        if not result:
            return "- [비교 실패] 비교 결과를 받지 못했습니다."
        return result

    def build_next_round_prompt(
        self, question: str, current_document: str, change_summary: str = ""
    ) -> str:
        """
        라운드가 끝날 때마다(확정 문서가 새로 만들어질 때마다) 호출해서,
        그 다음 라운드에 외부 AI에게 보낼 질문 문구를 만듦.

        기존 build_cross_check_prompt(v1.6)와의 차이: 그쪽은 "검증 결과
        텍스트"를 보여줬지만, 이 메서드는 "지금까지 만들어진 문서 전체"를
        그대로 보여줌. 외부 AI가 텍스트 조각이 아니라 완성된 문서를 보고
        교차검증하므로, 문서 전체에 걸친 문제(예: 법령 근거 목록에 더 이상
        맞지 않는 조항이 남아있는 것, 문장 중복 등)도 잡아낼 수 있음.
        ...
        current_document : str
            지금 시점의 확정 문서 전체 (v1, v2, v3...)
        change_summary : str, optional
            바로 이전 라운드에서 무엇을 고쳤는지 요약. 있으면 질문에 포함시켜,
            외부 AI가 "이미 반영된 사항"을 다시 지적하지 않고 새로운 부분에
            집중하게 함. 1라운드(아직 변경 이력이 없음)는 빈 문자열로 둠.
        ...
        change_block = ""
        if change_summary:
            change_block = f"\n\n[참고 — 바로 이전 라운드에서 이미 반영된 수정 사항]\n{change_summary}"

        return f"""아래는 세무 자문 AI 시스템이 작성한 답변 문서입니다. 이 문서 전체를
검토하고 교차검증해주세요.

[원래 질문]
{question}

[검토 대상 — 현재까지 작성된 답변 문서 전체]
{current_document}
{change_block}

[요청 사항]
1. 이 문서에 사실관계 오류(법령 조항 번호, 수치, 기준 금액, 단정적 결론 등)가
   있는지 검토해주세요.
2. 문서 전체의 일관성도 확인해주세요 — 예를 들어 같은 내용이 두 군데에
   다르게(또는 중복으로) 적혀있지 않은지, 한 항목을 고치면서 다른 항목과
   안 맞게 된 부분은 없는지(예: 법령 근거 목록에 본문 설명과 안 맞는 조항이
   남아있는 경우 등)도 확인해주세요.
3. 결론적으로 이 문서에서 실제로 고쳐야 할 부분이 무엇인지, 그리고 정확한
   근거(조항 번호, 정확한 수치 등)는 무엇인지 구체적으로 알려주세요.
4. 확실하지 않은 부분은 "확실하지 않음"이라고 솔직하게 말해주세요. 추측해서
   단정적으로 답하지 마세요.
5. 더 이상 고칠 부분이 없다고 판단되면, 그렇게 명확히 말해주세요.
"""

    def apply_external_feedback(
        self, question: str, current_document: str, external_ai_input: str
    ) -> tuple:
        """
        외부 AI의 (자유 형식) 답변을 반영해서, 현재 문서를 다음 버전으로
        다시 작성함. 웹검색은 사용하지 않음(회계사 피드백 반영 — 2라운드
        부터는 속도를 위해 외부 AI 의견만 근거로 함, 1라운드의 웹검색
        결과는 이미 본문에 반영되어 있으므로 의미가 유지됨).

        기존 _apply_corrections(v1.5)와의 차이: 그쪽은 정형화된 검증 텍스트
        ("- 항목 → 답변과 다름")에서 항목을 추출해 "원문 발췌-치환" 방식으로
        좁게 수정했음. 이 메서드는 외부 AI의 자유 형식 답변(표, 번호 목록,
        줄글 등 형식이 일정하지 않음)을 입력으로 받으므로, 그 방식을 쓸 수
        없음. 대신 "문서 전체 + 외부 AI 의견"을 모델에 주고, 문서 전체를
        다시 쓰게 함 — 단, 아래 안전장치로 과도한 창작 위험을 제한함:
        - 문서의 섹션 구조(①질의요지 ②결론 ③회계처리 ④법령근거 등)는
          그대로 유지하고, 내용만 외부 AI 의견을 반영해 고치도록 명시
        - 외부 AI가 "오류 아님/수정 불필요"라고 명시한 부분은 건드리지 않도록 명시
        - (2026-06-28 추가 — 회계사 피드백 반영: 새 조항 번호를 창작해 끼워
          넣는 사고 발견) 외부 AI가 "이 조항이 틀렸다"고만 하고 정확한 번호를
          제시하지 않았거나 "확실하지 않음"이라고 밝힌 경우, 모델이 스스로
          새 조항/수치를 만들어 채우지 않고 그 불확실성을 그대로 문서에
          반영(원문 유지 또는 "재확인 필요" 표시)하도록 명시
        - 무엇을 어떻게 고쳤는지 변경 요약을 반드시 같이 받아, 화면에서
          회계사가 그 라운드의 변경 내역을 검토할 수 있게 함
        ...
        current_document : str
            현재 버전의 문서 전체
        external_ai_input : str
            외부 AI로부터 받아온 자유 형식 답변

        Returns
        -------
        tuple[str, str]
            (new_document, change_summary)
            new_document: 외부 AI 의견을 반영해 다시 작성된 문서 전체
            change_summary: 이번 라운드에 무엇을 어떻게 고쳤는지 요약 (회계사가
                            검토하기 쉽도록 불릿 형식의 한국어 요약)
            호출 실패 시 new_document는 current_document를 그대로 반환하고,
            change_summary에 실패 사유를 담음 (문서를 잃어버리지 않도록 함).
        """
        prompt = f"""다음은 세무 자문 답변 문서와, 그 문서에 대해 다른 AI가 교차검증한
의견입니다. 이 의견을 반영해서 문서를 다시 작성해주세요.
...
[작업]
1. 위 의견에서 실제로 고쳐야 한다고 지적된 사항만 반영해서 문서를 다시
   작성하세요. 의견이 "오류 아님", "수정 불필요", "문제없음"이라고 명시한
   부분은 건드리지 마세요.
2. 문서의 섹션 구조(### 1. 질의 요지, ### 2. 세무 결론 및 판단, ### 3. 실무
   회계처리 지침(있는 경우), ### 4. 관련 근거 및 유의사항 등 원래 문서의
   섹션 제목과 순서)는 그대로 유지하세요. 섹션을 추가하거나 삭제하거나
   순서를 바꾸지 마세요.
3. 한 항목을 고칠 때, 그로 인해 문서의 다른 부분(예: 법령 근거 목록)이
   더 이상 맞지 않게 되지는 않는지 확인하고, 필요하면 그 부분도 일관되게
   함께 고치세요. 예를 들어 본문에서 어떤 조항이 틀렸다고 판단되어 다른
   조항으로 대체한다면, 법령 근거 목록에서도 그 틀린 조항을 완전히
   제거하고 맞는 조항으로 교체하세요 — 틀린 조항을 설명만 바꿔서 남겨두지
   마세요.
4. 매우 중요 — 새로운 사실을 만들어내지 마세요: 의견에서 "이 조항이
   틀렸다"고만 하고 정확한 조항 번호를 제시하지 않았다면, 절대로 스스로
   추측해서 새 조항 번호를 만들어 끼워 넣지 마세요. 의견이 "확실하지
   않음", "재확인 필요", "다시 확인하는 것을 권장합니다" 같은 식으로 직접
   불확실성을 밝힌 부분은, 그 불확실성을 그대로 문서에 반영하세요 — 예를
   들어 "(정확한 조문 번호는 재확인이 필요합니다)"라고 표시하거나, 원래
   있던 조항을 일단 그대로 두세요. 법령 조항 번호, 금액, 비율, 날짜 등
   구체적인 사실관계는 의견에 명시적으로 적힌 값만 사용하세요 — 의견이
   "이 설명을 더 명확히 표현하라"는 식으로 표현/구조 개선만 요구한 경우에도
   마찬가지로, 새로운 조항이나 수치를 임의로 만들어 추가하지 마세요.
5. 같은 내용을 중복해서 두 번 적지 마세요. 문장이 어색하게 끊기거나 두
   표현이 이상하게 이어붙지 않도록, 수정하는 부분은 자연스러운 하나의
   글로 다시 쓰세요.
6. 의견을 반영하면서도 원본의 전문가 답변체 어조와 전체적인 분량/상세도는
   유지하세요. 의견에서 언급되지 않은 부분은 원문을 그대로 보존하세요.
7. 무엇을 어떻게 고쳤는지 "change_summary"에 불릿("- ") 형식의 한국어로
   요약하세요(고친 게 없다면 "- 수정 사항 없음"). 각 불릿은 1줄 내외로
   간결하게 쓰세요. 4번 지침에 따라 불확실성을 그대로 남겨둔 경우, 그 점도
   "- (조항/수치)는 의견에서도 불확실하다고 하여 원문 유지, 재확인 필요로
   표시함"처럼 요약에 명시하세요.

[출력 형식 — 다른 설명 없이 이 JSON 형식만 출력하세요. 마크다운 코드블록
표시(```)도 쓰지 마세요. new_document 안의 줄바꿈은 \\n으로 표시하세요]
{{"new_document": "...", "change_summary": "..."}}
"""
        try:
            response = self.client.models.generate_content(
            ...
            return current_document, f"[수정 실패] 외부 AI 의견 반영 호출에 실패했습니다: {e}"

        if not raw_text:
            return current_document, "[수정 실패] 응답을 받지 못했습니다. 원본을 그대로 유지합니다."

        parsed = _parse_model_json(raw_text)
        if parsed is None:
            return (
                current_document,
                "[수정 실패] 결과 형식을 해석하지 못했습니다. 원본을 그대로 유지합니다.",
            )

        new_document = str(parsed.get("new_document", "")).strip()
        change_summary = str(parsed.get("change_summary", "")).strip()

        if not new_document:
            return current_document, "[수정 실패] 새 문서를 만들지 못했습니다. 원본을 그대로 유지합니다."

        return new_document, (change_summary or "- 수정 사항 없음")

    # ------------------------------------------------------------------
    # v1.11 신규 (2026-07-10) — Claude(Sonnet) 자동 교차검증
    # ------------------------------------------------------------------
    # 배경: v1.7까지의 교차검증은 "회계사가 다음 라운드 질문(build_next_round_prompt)을
    # 복사 → 외부 AI(ChatGPT 등) 사이트에 붙여넣기 → 답변을 복사 → 시스템에 붙여넣기
    # → apply_external_feedback로 본문 반영"을 사람이 왕복하는 반자동 구조였음.
    # 이 자리에서 사람의 복붙을 서로 다른 벤더인 Claude(Sonnet) API 호출로 대체해
    # 자동으로 여러 라운드를 돌린다. 저자 모델(Gemini)과 검증자(Claude)를 벤더까지
    # 분리해 독립적 교차검증이 되게 하는 것이 목적.
    #
    # 안전장치(개발계획서 회고 7.2 반영):
    # - 종료조건을 "Claude의 완료 판단" 하나에만 맡기지 않고, (1) Claude가
    #   [검토완료: 예] 신호를 냄 + (2) 본문 무변화 감지(difflib 유사도) +
    #   (3) 최대 라운드 상한 의 3중으로 둔다. AI가 과신해 헛돌거나 오답으로
    #   수렴한 채 "완료"를 선언해도 안전하게 멈춘다.
    # - 본문 재작성은 여전히 기존 apply_external_feedback(Gemini)가 담당한다.
    #   Claude는 "점검·지적"만 하고, 그 지적을 문서에 반영하는 것은 저자 모델이
    #   맡아 창작 방지 안전장치(새 조항/수치 임의 생성 금지)를 그대로 재사용한다.
    def _get_anthropic_client(self):
        """Anthropic 클라이언트를 지연 초기화해서 반환. 키/패키지 문제는 RuntimeError."""
        if self._anthropic_client is not None:
            return self._anthropic_client
        if not self.anthropic_api_key:
            raise RuntimeError(
                "ANTHROPIC_API_KEY가 설정되어 있지 않습니다. Streamlit Secrets(또는 "
                "로컬 .env)에 ANTHROPIC_API_KEY를 추가하세요."
            )
        try:
            import anthropic
        except ImportError as e:
            raise RuntimeError(
                "anthropic 패키지가 설치되어 있지 않습니다. requirements.txt에 "
                "anthropic를 추가하고 재배포(Streamlit Cloud reboot)하세요."
            ) from e
        self._anthropic_client = anthropic.Anthropic(
            api_key=self.anthropic_api_key, timeout=180.0
        )
        return self._anthropic_client

    def call_claude_verifier(self, prompt: str, enable_web_search: bool = False) -> tuple:
        """
        Claude(Sonnet)에게 세무 답변 문서를 교차검증시키고, 지적 텍스트와
        "더 고칠 것 없음" 종료 신호를 함께 받아온다.

        Parameters
        ----------
        prompt : str
            Claude에게 보낼 사용자 메시지. 보통 build_next_round_prompt가 만든
            "현재 문서 전체 + 요청사항"을 그대로 넘긴다.
        enable_web_search : bool
            웹검색 도구 사용 여부. 기본값 False(비용 절감 — 웹검색은 호출당
            별도 과금되고 검색 결과 토큰까지 얹혀 비용이 크게 늘어난다).
            True면 조문으로 부족할 때 Claude가 스스로 국세청·법제처 등을
            검색하도록 허용한다. 도구가 거부되면 자동으로 도구 없이 한 번 더
            시도한다. 조문 지식만으로도 핵심 오류(조항 번호·특례 규정 등)는
            대부분 잡히므로, 최근 개정 시점 확인 등이 꼭 필요할 때만 켠다.

        Returns
        -------
        tuple[str, str, str, str]
            (critique_text, grade, reason, error)
            critique_text : Claude의 검증 의견 전체 텍스트
            grade         : [검토판정] 등급 'S'/'A'/'B'/'C'/'D'/'F' (파싱 실패 시 '?')
            reason        : [한줄사유] 등급 판정 이유 한 문장
            error         : 실패 시 사유(성공 시 빈 문자열; 이때 나머지는 빈 문자열)
        """
        system_instruction = (
            "당신은 한국 세무 답변을 점검하는 독립 검증자입니다. 사용자 메시지로 "
            "주어지는 답변 문서에 사실관계 오류나 법령 오류가 있는지 확인하세요.\n\n"
            "■ 근거 판단 우선순위 (반드시 이 순서를 지킬 것)\n"
            "1) 관련 법령의 조문 원문을 최우선 근거로 삼는다. 조문 내용을 정확히 "
            "알고 있으면 그것으로 판단한다.\n"
            "2) 조문만으로 판단이 부족하거나 개정 여부·적용 시기가 불확실하면 그때 "
            "웹검색을 사용한다. 웹검색 시에는 국세청(nts.go.kr)·법제처(law.go.kr) 등 "
            "1차 공식 출처를 우선하고, 블로그·카페 등 2차 출처는 신뢰하지 않는다.\n"
            "3) 조문과 검색으로도 확실하지 않으면, 조항 번호나 수치를 추측으로 "
            "만들어내지 말고 '확실하지 않음 — 재확인 필요'라고 명시한다.\n\n"
            "■ 점검 방법\n"
            "- 문서의 핵심 주장(기준금액, 적용 시기, 조항 번호, 발급/수취·매출/매입 "
            "방향, 회계처리 등)을 하나씩 조문에 비추어 확인한다.\n"
            "- 오류가 있으면 항목마다: 틀린 부분 인용 / 무엇이 맞는지 / 근거(조문명·조 "
            "번호 또는 출처) / 근거 출처 구분([조문]/[웹검색]/[불확실]) / 확신도(확실/"
            "추정/불확실)를 밝힌다.\n"
            "- 맞는 내용을 억지로 틀렸다고 뒤집지 말 것. 정확한 부분은 그대로 두라.\n"
            "- 조항 번호·수치를 추측으로 지어내지 말 것.\n\n"
            "■ 출력 마지막 (매우 중요, 형식 고정)\n"
            "검토를 마치면 반드시 응답의 맨 마지막에 아래 두 줄을 정확히 이 형식으로 "
            "출력하라.\n"
            "[검토판정: X]   (X는 아래 S/A/B/C/D/F 중 하나)\n"
            "[한줄사유: ...] (그 등급으로 판정한 핵심 이유를 한 문장으로)\n\n"
            "등급 기준 — 문서의 '현재 완성도'와 '추가 검토로 더 나아질 여지'를 함께 "
            "고려해 하나만 고른다:\n"
            "· S = 남은 지적이 전혀 없음. 더 볼 것이 없는 완성 상태.\n"
            "· A = 확정해도 됨. 완벽하진 않으나 사실관계·결론·수치·조항이 모두 맞고 "
            "남은 것은 무시할 만한 표현 문제뿐.\n"
            "· B = 확정해도 됨. 다만 남은 사항(예: 조항번호·시행일 재확인 등)은 사람이 "
            "법제처·국세청 원문으로 확인해야 하는 것이라, 검증을 더 반복해도 실질적으로 "
            "나아지지 않는 상태.\n"
            "· C = 확정해도 큰 문제는 없으나, 한 번 더 다듬으면 실제로 더 정확해질 여지가 "
            "남아 있는 상태(개선 실익 있음).\n"
            "· D = 실질적인 오류가 있어 한 번 더 수정이 필요함(치명적이지는 않음).\n"
            "· F = 결론이나 핵심 수치·조항이 틀림. 반드시 수정해야 하며 이대로 확정하면 "
            "안 됨.\n"
            "판정이 애매하면 '더 안전한 쪽(더 낮은 등급)'으로 내려 잡는다. 특히 결론이 "
            "틀릴 가능성이 있으면 주저 없이 F 또는 D로 판정하라."
        )
        ...
        def _do_call(with_tools: bool):
            kwargs = dict(
                model=self.claude_model,
                # v1.11.4: 종합문서(여러 질의 통합)처럼 원래 문서 자체가 긴 경우
                # 8192로도 검증 의견이 끝까지 안 나오고 끊기는 사례 확인되어 재상향.
                # (2라운드 이후 프롬프트에는 문서 전체+이전 지적까지 누적됨)
                max_tokens=16000,
        ...
        def _call_once():
            """한 번 호출해서 (critique, stop_reason, error) 반환. 실패 시 error만 채움."""
            try:
                try:
                    resp = _do_call(with_tools=True)
                except Exception:
                    if enable_web_search:
                        resp = _do_call(with_tools=False)
                    else:
                        raise
            except Exception as e:
                msg = str(e)
                low = msg.lower()
                hint = ""
                if "model" in low and ("not found" in low or "not_found" in low or "does not exist" in low):
                    hint = (
                        f" (모델 문자열 '{self.claude_model}'을(를) 찾을 수 없습니다. "
                        "Streamlit Secrets의 CLAUDE_MODEL을 docs.claude.com에 안내된 현재 "
                        "Sonnet 모델 문자열로 지정하세요.)"
                    )
                elif "authentication" in low or "api key" in low or " 401" in low or "invalid x-api-key" in low:
                    hint = " (ANTHROPIC_API_KEY가 올바른지 확인하세요.)"
                return "", "", f"[Claude 검증 실패] {msg}{hint}"
            ...
            return crit, stop_reason, ""

        # 첫 시도. 빈 응답이면 진단 메시지를 만들고, 명백한 API 오류가 아니라면
        # (요청 자체는 성공했는데 텍스트만 비어 돌아온 경우) 과금이 이미 발생했을
        # 수 있으므로 낭비를 줄이기 위해 딱 한 번만 자동 재시도한다.
        critique, stop_reason, err = _call_once()
        if err:
            return "", "", "", err

        if not critique:
            if stop_reason == "max_tokens":
                # 재시도해도 같은 이유로 또 잘릴 가능성이 높으므로 재시도하지 않고
                # 원인을 명확히 알린다.
                return "", "", "", (
                    "[Claude 검증 실패] 응답이 max_tokens 한도 안에서 텍스트를 하나도 "
                    "만들지 못하고 끊겼습니다. 문서가 매우 길 때 발생할 수 있습니다."
                )
            # 원인 불명의 빈 응답 — 일시적 문제일 수 있으므로 1회만 재시도
            critique, stop_reason, err = _call_once()
            if err:
                return "", "", "", err
            if not critique:
                return "", "", "", (
                    f"[Claude 검증 실패] 빈 응답을 받았습니다(재시도 1회 포함, "
                    f"stop_reason={stop_reason or '알 수 없음'}). API 호출 자체는 "
                    "완료되어 이미 과금됐을 수 있습니다 — 콘솔 Billing에서 사용량을 "
                    "확인해보세요. 문제가 반복되면 최대 라운드를 1로 낮추거나 "
                    "잠시 후 다시 시도해보세요."
                )

        # 등급 판정 마커 파싱 — 마지막 등장분 기준
        # [검토판정: X]  (X ∈ S/A/B/C/D/F)  + [한줄사유: ...]
        import re
        grade = ""
        reason = ""
        gm = re.findall(r"\[검토판정\s*[:：]\s*([SABCDFsabcdf])\s*\]", critique)
        if gm:
            grade = gm[-1].upper()
        rm = re.findall(r"\[한줄사유\s*[:：]\s*(.+?)\]", critique, re.DOTALL)
        if rm:
            reason = rm[-1].strip()
        # 등급이 안 잡히면(형식 위반) 안전하게 '사람이 직접 판단'으로 처리 — 물음표.
        # 자동 확정도, 무한 재시도도 하지 않고 회계사에게 넘긴다.
        if grade not in ("S", "A", "B", "C", "D", "F"):
            grade = "?"

        return critique, grade, reason, ""

    def run_auto_cross_check(
        self,
        question: str,
        current_document: str,
        max_rounds: int = 2,
        convergence_ratio: float = 0.995,
        prev_change_summary: str = "",
        enable_web_search: bool = False,
        on_step=None,
    ) -> dict:
        """
        Claude(Sonnet) 검증자로 자동 교차검증 루프를 돈다.

        각 라운드:
          1) build_next_round_prompt로 "현재 문서 전체 + 요청사항" 질문 생성
          2) call_claude_verifier로 Claude에게 점검 요청 → 6등급(S/A/B/C/D/F) 판정
          3) S/A/B/? → 본문 변경 없이 종료(회계사 확정/판단)
          4) C → 지적을 1회 반영하고 종료(회계사 판단)
          5) D/F → 지적을 반영하고 다음 라운드로 계속
          6) 반영 실패(apply_failed)·개선 정체(stalled)·최대 라운드(max_rounds)면 종료
        ...
        question : str
            원래 질문
        current_document : str
            자동 교차검증을 시작할 시점의 문서 전체(보통 1차 확정 문서)
        max_rounds : int
            Claude 호출 최대 횟수(상한)
        convergence_ratio : float
            difflib 유사도가 이 값 이상이면 "본문 무변화"로 간주하고 종료
        prev_change_summary : str
            직전 라운드까지의 변경 요약(첫 자동 라운드 질문에 참고로 포함됨)
        on_step : callable, optional
            매 라운드(본문이 새로 만들어진 경우) 완료 시 그 step dict를 인자로
            호출됨. 화면 갱신/백업 등 부수 작업용. 예외는 무시됨.

        Returns
        -------
        dict
            {
              "steps": list[dict],   # 본문이 새로 생성된 라운드들
                                     #   각 dict: prompt_sent, critique, new_document,
                                     #            change_summary, similarity, grade
              "stop_reason": str,    # "graded_stop"(등급에 따른 정상 종료) |
                                     #   "apply_failed" | "max_rounds" | "error"
              "final_grade": str,    # 마지막 라운드의 등급 S/A/B/C/D/F 또는 '?'
              "final_reason": str,   # 그 등급 판정의 한줄사유
              "final_critique": str, # 마지막 Claude 검증 텍스트
              "error": str,          # 실패 시 사유
              "rounds_run": int,     # 실제 Claude 호출 횟수
            }

        종료/진행 규칙 (등급 기반):
          · S, A → 확정 가능. 즉시 종료(자동 확정 권유).
          · B, C → 확정 가능하나 회계사 판단 필요. 즉시 종료(회계사가 확정/재실행 결정).
                   B는 "더 돌려도 개선 실익 낮음", C는 "한 번 더 다듬으면 나아질 여지".
          · D, F → 실질 오류. Gemini로 지적을 반영하고 다음 라운드로 계속(최대 라운드 상한).
          · '?' (등급 파싱 실패) → 안전하게 회계사 판단으로 넘김(종료, 경고).
        """
        result = {
            "steps": [],
            "stop_reason": "",
        ...
        if not self.auto_cross_check_enabled:
            result["stop_reason"] = "error"
            result["error"] = (
                "ANTHROPIC_API_KEY가 설정되어 있지 않아 자동 교차검증을 실행할 수 "
                "없습니다. Streamlit Secrets에 ANTHROPIC_API_KEY를 추가하세요."
            )
            return result

        doc = current_document
        prev_summary = prev_change_summary

        for _ in range(max(1, max_rounds)):
            prompt = self.build_next_round_prompt(question, doc, prev_summary)
            critique, grade, reason, err = self.call_claude_verifier(
                prompt, enable_web_search=enable_web_search
            )
            result["rounds_run"] += 1
            if critique:
            ...
            # S/A/B/? → 확정 가능 또는 회계사 판단 단계. 본문을 더 고치지 않고
            # 여기서 종료한다(B는 '더 돌려도 실익 낮음', ?는 판정 실패).
            if grade in ("S", "A", "B", "?"):
                result["stop_reason"] = "graded_stop"
                break

            # C/D/F → Claude 지적을 Gemini로 본문에 반영한다.
            new_doc, summary = self.apply_external_feedback(question, doc, critique)
            ratio = difflib.SequenceMatcher(None, doc or "", new_doc or "").ratio()

            # 반영 실패 감지: apply_external_feedback는 실패 시 원본을 그대로
            # 돌려주고 change_summary 앞에 "[수정 실패]"를 붙인다. 이 경우 문서가
            # 안 바뀌므로, 이를 정상 종료로 처리하면 "지적이 반영 안 됐는데 성공
            # 으로 표시"되는 위험이 생긴다. 별도 종료사유로 구분해 경고한다.
            apply_failed = summary.strip().startswith("[수정 실패]")
            ...
            if apply_failed:
                result["stop_reason"] = "apply_failed"
                break
            ...
            if grade == "C":
                # 개선 여지가 있어 한 번 반영했다. 여기서 멈추고 회계사가 확정할지
                # 한 번 더 돌릴지 결정하게 한다(무한 반복 방지).
                result["stop_reason"] = "graded_stop"
                break

            # D/F → 다음 라운드로 계속. 단, 반영해도 문서가 사실상 안 바뀌면
            # (개선 정체) 무한 반복을 막기 위해 멈추고 회계사에게 넘긴다.
...
"""
        ...
        parsed = _parse_model_json(raw_text)
        if parsed is None:
            print(f"[경고] 항목별 수정 결과 JSON 파싱 실패 / 원본: {raw_text[:200]}")
            return {"original_snippet": "", "replacement": "", "short_reason": ""}
...
"""
        ...
        parsed = _parse_model_json(raw_text)
        if parsed is None:
            print(f"[경고] 키워드 기반 단락 재작성 결과 JSON 파싱 실패 / 원본: {raw_text[:200]}")
            return {"target_paragraph": "", "new_paragraph": "", "short_reason": ""}
...
"""
    ...
    # ------------------------------------------------------------------
    def extract_keywords_and_gist(self, question: str, confirmed_content: str) -> tuple:
        """
        확정된 지식 문서에서 검색용 '키워드'와 한 문장 '요지'를 뽑는다.
        조회 화면에서 "첫 문장이 잘린 것" 대신 진짜 취지를 보여주기 위한 보조 호출.

        - 주엔진(Gemini Flash)로 추출한다(별도 벤더/웹검색 없음, 매우 저렴).
        - 실패해도 저장을 막지 않도록 ("", "")를 안전하게 반환한다.

        Returns
        -------
        tuple[str, str]
            (keywords, gist)
            keywords : 쉼표로 구분한 검색 키워드 (예: "가지급금, 인정이자, 가중평균차입이자율")
            gist     : 핵심 결론을 담은 한 문장
        """
        if not confirmed_content or not confirmed_content.strip():
            return "", ""

        # 너무 긴 본문은 앞부분 위주로 잘라 비용/시간 절약(요지·키워드엔 충분)
        body = confirmed_content.strip()
        if len(body) > 6000:
            body = body[:6000]

        extract_prompt = f"""다음은 회계법인에서 확정한 세무 자문 문서입니다. 이 문서를 나중에 빠르게
검색하고 취지를 파악할 수 있도록, 아래 두 가지만 뽑아주세요.

1) 키워드: 이 문서를 대표하는 핵심 세무 용어 5~8개 (조문명·제도명·쟁점 위주).
   쉼표로 구분한 한 줄로. 일반적이고 흔한 단어(예: 세금, 신고)는 피하고 변별력 있는 용어를 고르세요.
2) 요지: 이 문서의 결론·핵심을 담은 한 문장. 질의 요지가 아니라 '결론/판단'을 요약하세요.
   40자 내외로 간결하게.
...
[출력 형식 — 다른 설명 없이 이 JSON만 출력. 마크다운 코드블록(```)도 쓰지 마세요]
{{"키워드": "...", "요지": "..."}}"""
        ...
        parsed = _parse_model_json(raw_text)
        if parsed is None:
            print(f"[경고] 키워드/요지 JSON 파싱 실패 / 원본: {raw_text[:200]}")
            return "", ""

        keywords = str(parsed.get("키워드", "") or "").strip()
        gist = str(parsed.get("요지", "") or "").strip()
        return keywords, gist
    ...
    def confirm_to_knowledge_base(
        ...
        """
        if self.sheet_logger and self.sheet_logger.enabled:
            # 확정과 동시에 검색용 키워드·요지를 뽑아 함께 저장(조회 화면에서 취지 표시).
            # 실패해도 저장은 계속되도록 안전 처리.
            try:
                kw, gist = self.extract_keywords_and_gist(question, confirmed_content)
            except Exception:
                kw, gist = "", ""
