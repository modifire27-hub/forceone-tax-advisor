...
"""
    ...
    def call_claude_verifier(self, prompt: str, enable_web_search: bool = False) -> tuple:
        ...
        try:
            try:
                response = _do_call(with_tools=True)
            except Exception:
                # 웹검색 도구를 모델/SDK가 거부하면 도구 없이 한 번 더 시도
                if enable_web_search:
                    response = _do_call(with_tools=False)
                else:
                    raise
        except Exception as e:
            msg = str(e)
            ...
            return "", False, f"[Claude 검증 실패] {msg}{hint}"

        # 텍스트 블록만 이어붙임 (web_search 결과 등 비텍스트 블록은 제외)
        parts = []
        ...
        if not critique:
            return "", False, "[Claude 검증 실패] 빈 응답을 받았습니다."
