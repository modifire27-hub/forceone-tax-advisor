...
"""
    ...
    def call_claude_verifier(self, prompt: str, enable_web_search: bool = False) -> tuple:
        ...
        -------
        tuple[str, bool, str]
            (critique_text, is_complete, error)
            critique_text : Claude의 검증 의견 전체 텍스트
            is_complete   : Claude가 [검토완료: 예]로 판단했는지 여부
            error         : 실패 시 사유(성공 시 빈 문자열)
        """
        system_instruction = (
            ...
            "■ 출력 마지막 줄 (매우 중요, 형식 고정)\n"
            "검토를 마치면 반드시 응답의 맨 마지막 줄에 아래 형식으로 판단을 표시하라. "
            "더 고쳐야 할 실질적 오류가 남아 있으면 '아니오', 더 이상 고칠 실질적 "
            "오류가 없으면 '예'로 표시한다(표현·서술을 다듬는 사소한 제안만 남은 "
            "경우도 '예'로 본다).\n"
            "[검토완료: 예]  또는  [검토완료: 아니오]"
        )
        ...
        # 종료 판단 마커 파싱 — 마지막 등장분 기준
        import re
        is_complete = False
        matches = re.findall(
            r"\[검토완료\s*[:：]\s*(예|아니오|yes|no)\s*\]", critique, re.IGNORECASE
        )
        if matches:
            last = matches[-1].strip().lower()
            is_complete = last in ("예", "yes")

        return critique, is_complete, ""
    ...
    def run_auto_cross_check(
        ...
        각 라운드:
          ...
          2) call_claude_verifier로 Claude에게 점검 요청
          3) Claude가 [검토완료: 예] → 본문 변경 없이 종료(claude_complete)
          4) 아니면 apply_external_feedback(Gemini)로 지적을 본문에 반영
          5) 직전 문서와 유사도가 convergence_ratio 이상이면 무변화로 보고 종료(converged)
          6) max_rounds에 도달하면 종료(max_rounds)
        ...
        dict
            {
              "steps": list[dict],   # 본문이 새로 생성된 라운드들
                                     ...
                                     #            change_summary, similarity
              "stop_reason": str,    # "claude_complete"(성공) | "converged"(무변화-미완료) |
                                     #   "apply_failed"(반영 실패) | "max_rounds" | "error"
              "final_critique": str, # 마지막 Claude 검증 텍스트(완료 판단 포함)
              "error": str,          # 실패 시 사유
        ...
        for _ in range(max(1, max_rounds)):
            ...
            critique, is_complete, err = self.call_claude_verifier(
            ...
            if is_complete:
                # Claude가 "더 고칠 실질 오류 없음"으로 판단 → 본문 변경 없이 종료
                result["stop_reason"] = "claude_complete"
                break
            ...
            # 반영 실패 감지 (v1.11.1 안전 수정):
            # apply_external_feedback는 실패 시 원본 문서를 그대로 돌려주고
            # change_summary 앞에 "[수정 실패]"를 붙인다. 이 경우 문서가 안
            # 바뀌므로 유사도가 1.0이 되는데, 이걸 "수렴(정상 종료)"으로
            # 처리하면 "Claude가 지적한 오류가 반영 안 됐는데 성공(✅)으로
            # 표시"되는 위험한 상황이 된다(회고 사례). 반드시 별도 종료사유로
            # 구분해서 회계사에게 경고해야 한다.
            ...
            if apply_failed:
                # Claude는 지적했으나 본문 반영에 실패 → 성공 아님, 즉시 경고 종료
            ...
            if ratio >= convergence_ratio:
                # 문서가 더 이상 바뀌지 않음. 단, Claude가 [검토완료: 예]를
                # 낸 게 아니라 "그냥 안 바뀐" 상태이므로 완전한 성공이 아니다.
                # UI에서 ✅가 아니라 ⚠️로 표시하고 Claude 최종 의견을 확인하게 함.
